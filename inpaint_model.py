...
def SINet(x, mask, padding='SAME', name='inpaint_net', reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        x13 = region_deconv(x_complete, x_missing, mask,
                            name='com_13')
        x_complete, x_missing = tf.concat([x13, x3], axis=-1), x13
        x14 = region_conv(x_complete, x_missing, mask,
                          name='com_14')

        x_complete, x_missing = tf.concat([x14, x2], axis=-1), x14
        x15 = region_deconv(x_complete, x_missing, mask,
                            name='com_15')
...
def build_graph_with_loss(batch_data, batch_size, mask, vgg_path, adv_type, stage=0,
    ...
    rec_loss = tf.reduce_sum(tf.abs(image_gt - image_p1)) \
        + tf.reduce_sum(tf.abs(image_gt - image_p2))
    ...
    if stage == 1:
        ...
        shape = tf.shape(d_pred)
        mask_label = tf.image.resize_nearest_neighbor(mask_label,
                                                      [shape[1], shape[2]])
    ...
    d_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'discriminator') if stage == 1 \
        else None
