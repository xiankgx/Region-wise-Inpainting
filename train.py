...
if __name__ == '__main__':
    ...
    mask = tf.placeholder(tf.float32, [batch_size, args.height, args.width, 1],
                          name='mask')
    ...
    for j in range(0, args.epoch):
        for i in range(Iters):
            ...
            if args.mask_type == 0:
                ...
                mask_ = np.stack([continuous_mask(args.width, args.height, num, 360, 32, 50)
                                  for _ in range(batch_size)],
                                 axis=0)
            ...
            if i % 20 == 0:
                if args.stage == 0:
                    print(
                        '[{}/{}] rec_loss: {} correlation_loss:{} style_loss: {}'.format(i, Iters,
                                                                                         rec,
                                                                                         closs,
                                                                                         sloss))
                else:
                    print('[{}/{}] g_loss: {} rec_loss: {} correlation_loss:{} style_loss: {} d_loss: {}'.format(i, Iters,
                                                                                                                 g,
                                                                                                                 rec,
                                                                                                                 closs,
                                                                                                                 sloss,
        ...
        saver.save(sess, args.output + '/model/v{}.ckpt'.format(j),
                   global_step=Iters)
